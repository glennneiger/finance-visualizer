...
export default class TransactionService {
  ...
  //TODO add this ignore category name as a default drop down selection.
  ...
  rawGetTransactions = async (startDate, endDate) => {
    return new Promise((resolve, reject) => {
      fetch("api/transactions?startDate=" + startDate.format() + "&endDate=" + endDate.format())
        .then(
          async (json) => {
  ...
  getTransactionsByDateRange = async (startDate, endDate, forceReload) => {
    if (!forceReload && ((!startDate || !endDate) || (startDate.isSame(this.cache.startDate) && endDate.isSame(this.cache.endDate)))) {
  ...
  getTransactionSummaryByDateRange = async (startDate, endDate, forceReload) => {
    //get the data
    let transactionData = await this.getTransactionsByDateRange(startDate, endDate, forceReload).catch(
      (error) => {
    ...
    try {
      transactions.forEach((entry) => {
        if (entry.category == this.IGNORE_CATEGORY_NAME) {
          console.log("Ignoring transaction for $" + entry.amount);
          return;
        }
      ...
      transactions.forEach((entry) => {
        if (entry.category == category) {
          ...
          response.push({
            ...
            categorizationId: entry.categorizationId,
            source: entry.source
          });
        }
      });
